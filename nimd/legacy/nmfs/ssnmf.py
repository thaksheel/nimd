...
class SSNMF_N:
    ...
    def __init__(self, X, k, **kwargs):
        ...
        self.W = kwargs.get(
            "W", np.ones((rows, cols))
        )  # initialize missing data indicator matrix
        self.A = kwargs.get("A", np.random.rand(
            rows, k))  # initialize factor A
        self.S = kwargs.get("S", np.random.rand(
            k, cols))  # initialize factor S
        ...
        if self.Y is not None:
            ...
            if np.shape(self.Y)[1] != np.shape(self.X)[1]:
                raise Exception(
                    "The column dimensions of X and Y are not equal.")

            classes = np.shape(self.Y)[0]
            self.B = kwargs.get("B", np.random.rand(classes, k))
            self.lam = kwargs.get("lam", 1)
            self.L = kwargs.get(
                "L", np.ones((classes, cols))
            )  # initialize missing label indicator matrix
    ...
    def mult(self, **kwargs):
        ...
        for i in range(numiters):
            ...
            if self.modelNum == 1:
                self.A = self.dictupdateFro(
                    self.X, self.A, self.S, self.W, eps)
                self.S = np.transpose(
                    self.dictupdateFro(
                        np.transpose(self.X),
                        np.transpose(self.S),
            ...
            if self.modelNum == 3:
                self.A = self.dictupdateFro(
                    self.X, self.A, self.S, self.W, eps)
                self.B = self.dictupdateFro(
                    self.Y, self.B, self.S, self.L, eps)
                self.S = self.repupdateFF(eps)

                previousErr = currentErr
                currentErr = self.fronorm(
                    self.X, self.A, self.S, self.W
                ) ** 2 + self.lam * (self.fronorm(self.Y, self.B, self.S, self.L) ** 2)
                if i == 0:
                    initialErr = currentErr
            ...
            if self.modelNum == 4:
                ...
                currentErr = self.fronorm(
                    self.X, self.A, self.S, self.W
                ) ** 2 + self.lam * self.Idiv(self.Y, self.B, self.S, self.L)
                if i == 0:
                    initialErr = currentErr
            ...
            if self.modelNum == 5:
                ...
                previousErr = currentErr
                currentErr = self.Idiv(self.X, self.A, self.S, self.W) + self.lam * (
                    self.fronorm(self.Y, self.B, self.S, self.L) ** 2
                )
                if i == 0:
                    initialErr = currentErr
            ...
            if self.modelNum == 6:
                ...
                previousErr = currentErr
                currentErr = self.Idiv(
                    self.X, self.A, self.S, self.W
                ) + self.lam * self.Idiv(self.Y, self.B, self.S, self.L)
                if i == 0:
                    initialErr = currentErr
            ...
            if saveerrs:
                ...
                if self.modelNum == 3:
                    reconerrs.append(self.fronorm(
                        self.X, self.A, self.S, self.W))
                    classerrs.append(self.fronorm(
                        self.Y, self.B, self.S, self.L))
                    errs.append(reconerrs[i] ** 2 +
                                self.lam * classerrs[i] ** 2)
                ...
                if self.modelNum == 5:
                    ...
                    classerrs.append(self.fronorm(
                        self.Y, self.B, self.S, self.L))
                    errs.append(
                        reconerrs[i] + self.lam * (classerrs[i] ** 2)
                    )  # save errors
                    classaccs.append(self.accuracy())
                if self.modelNum == 6:
                    reconerrs.append(self.Idiv(self.X, self.A, self.S, self.W))
                    classerrs.append(self.Idiv(self.Y, self.B, self.S, self.L))
                    errs.append(reconerrs[i] + self.lam *
                                classerrs[i])  # save errors
                    classaccs.append(self.accuracy())
    ...
    def dictupdateIdiv(self, Z, D, R, M, eps):
        ...
        """
        return np.multiply(
            np.divide(D, eps + M @ np.transpose(R)),
            np.multiply(np.divide(np.multiply(M, Z),
                        eps + np.multiply(M, D @ R)), M)
            @ np.transpose(R),
        )
    ...
    def repupdateFF(self, eps):
        ...
        if np.min(self.S) == 0:
            self.S[self.S == 0] = 0.001 * np.max(self.S)
        if np.min(self.S) < 0:
            self.S = np.maximum(self.S, 0.001)
        S = np.multiply(
            np.divide(
                self.S,
                eps
                + np.transpose(self.A) @ np.multiply(self.W, self.A @ self.S)
                + self.lam
                * np.transpose(self.B)
                @ np.multiply(self.L, self.B @ self.S),
            ),
            np.transpose(self.A) @ np.multiply(self.W, self.X)
            + self.lam * np.transpose(self.B) @ np.multiply(self.L, self.Y),
        )
    ...
    def repupdateIF(self, eps, modelNum):
        ...
        if modelNum == 4:
            return np.multiply(
                np.divide(
                    ...
                    + (
                        2 *
                        np.transpose(self.A) @ np.multiply(self.W,
                                                           self.A @ self.S)
                        + self.lam * np.transpose(self.B) @ self.L
                    ),
                ),
                2 * np.transpose(self.A) @ np.multiply(self.W, self.X)
                + self.lam
                * np.transpose(self.B)
                @ np.multiply(
                    np.divide(
                        np.multiply(self.L, self.Y),
                        eps + np.multiply(self.L, self.B @ self.S),
                    ),
                    self.L,
                ),
            )
        ...
        if modelNum == 5:
            return np.multiply(
                ...
                @ np.multiply(
                    np.divide(
                        np.multiply(self.W, self.X),
                        eps + np.multiply(self.W, self.A @ self.S),
                    ),
                    self.W,
                )
                + 2 * self.lam *
                np.transpose(self.B) @ np.multiply(self.L, self.Y),
            )
    ...
    def accuracy(self, **kwargs):
        ...
        if Y is None:
            raise Exception(
                "Label matrix Y not provided: model is not supervised.")
    ...
    def Idiv(self, Z, D, S, M, **kwargs):
        ...
        div = (
            np.multiply(
                np.multiply(M, Z),
                np.log(np.divide(np.multiply(M, Z) + eps, Zhat + eps)),
            )
            - np.multiply(M, Z)
            + Zhat
        )
...
class SSNMF_T:
    ...
    def __init__(self, X, k, **kwargs):
        ...
        self.modelNum = kwargs.get("modelNum", 1)  # initialize model indicator
        self.W = kwargs.get(
            "W", torch.ones([rows, cols], dtype=torch.float, device=cuda)
        )  # initialize missing data indicator matrix
        self.A = kwargs.get(
            "A", torch.rand(rows, k, dtype=torch.float, device=cuda)
        )  # initialize factor A
        self.S = kwargs.get(
            "S", torch.rand(k, cols, dtype=torch.float, device=cuda)
        )  # initialize factor S
        self.tol = kwargs.get("tol", 1e-4)  # initialize factor tol
        ...
        if self.Y is not None:
            ...
            self.B = kwargs.get(
                "B", torch.rand(classes, k, dtype=torch.float, device=cuda)
            )
            self.lam = kwargs.get("lam", 1)
            self.L = kwargs.get(
                "L", torch.ones([classes, cols],
    ...
    def dictupdateIdiv(self, Z, D, R, M, eps):
        ...
        """
        return torch.mul(
            torch.div(D, eps + M @ torch.t(R)),
            torch.mul(torch.div(torch.mul(M, Z), eps + torch.mul(M, D @ R)), M)
            @ torch.t(R),
        )
    ...
    def repupdateFF(self, eps):
        ...
        return torch.mul(
            torch.div(
                self.S,
                eps
                + torch.t(self.A) @ torch.mul(self.W, self.A @ self.S)
                + self.lam *
                torch.t(self.B) @ torch.mul(self.L, self.B @ self.S),
            ),
            torch.t(self.A) @ torch.mul(self.W, self.X)
            + self.lam * torch.t(self.B) @ torch.mul(self.L, self.Y),
        )
        ...
        if modelNum == 4:
            return torch.mul(
                torch.div(
                    ...
                    eps
                    + (
                        2 * torch.t(self.A) @ torch.mul(self.W,
                                                        self.A @ self.S)
                        + self.lam * torch.t(self.B) @ self.L
                    ),
                ),
                2 * torch.t(self.A) @ torch.mul(self.W, self.X)
                + self.lam
                * torch.t(self.B)
                @ torch.mul(
                    torch.div(
                        torch.mul(self.L, self.Y),
                        eps + torch.mul(self.L, self.B @ self.S),
                    ),
                    self.L,
                ),
            )
        ...
        if modelNum == 5:
            return torch.mul(
                torch.div(
                    self.S,
                    eps
                    + torch.t(self.A) @ self.W
                    + 2
                    * self.lam
                    * torch.t(self.B)
                    @ torch.mul(self.L, self.B @ self.S),
                ),
                torch.t(self.A)
                @ torch.mul(
                    torch.div(
                        torch.mul(self.W, self.X),
                        eps + torch.mul(self.W, self.A @ self.S),
                    ),
                    self.W,
                )
    ...
    def repupdateII(self, eps):
        ...
        return torch.mul(
            torch.div(
                self.S,
                eps + torch.t(self.A) @ self.W + self.lam *
                torch.t(self.B) @ self.L,
            ),
            torch.t(self.A)
            @ torch.mul(
                torch.div(
                    torch.mul(self.W, self.X), eps +
                    torch.mul(self.W, self.A @ self.S)
                ),
                self.W,
            )
            + self.lam
    ...
    def Idiv(self, Z, D, S, M, **kwargs):
        ...
        if Z is None:
            raise Exception("Matrix Z not provided.")
        if M is None:
            M = torch.ones(Z.size(0), Z.size(
                1), dtype=torch.float, device=Z.device)

        # compute divergence
        Zhat = torch.mul(M, D @ S)
        div = (
            torch.mul(
                torch.mul(M, Z), torch.log(
                    torch.div(torch.mul(M, Z) + eps, Zhat + eps))
            )
            - torch.mul(M, Z)
            + Zhat
        )
